        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg  rounded-xl">
                <div class="px-4 py-4 flex justify-between items-center">
                    <div class="text-lg font-semibold text-[#5A827E] dark:text-[#5A827E]">Data Laporan Kembali</div>
                    <button
                        onclick="return filterData()"
                        class="bg-[#5A827E] hover:bg-[#4b6f6b] text-white px-4 py-2 rounded-md font-medium transition shadow">
                        Filter ReportKembali
                    </button>
                </div>
                    ...
                    <table id="myDataTable" class="min-w-full border border-[#5A827E] dark:border-[#5A827E] table-auto rounded-lg">
                        <thead class="bg-[#5A827E] text-white rounded-t-lg">
                            <tr>
                                ...
                                <th class="border border-green-700 px-4 py-2">No Transaksi Pinjam</th>
                                <th class="border border-green-700 px-4 py-2">Kode Anggota</th>
                                <th class="border border-green-700 px-4 py-2">Anggota</th>
                                <th class="border border-green-700 px-4 py-2">Tanggal Pinjam</th>
                                <th class="border border-green-700 px-4 py-2">Batas Pinjam</th>
                                <th class="border border-green-700 px-4 py-2">Kode Koleksi</th>
                                <th class="border border-green-700 px-4 py-2">Judul</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
$no = 1;
                            ...
                            @else
                                @foreach ($data as $d)
                                    <tr class="hover:bg-green-100 dark:hover:bg-green-900 transition">
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2 text-center">{{ $no++ }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->no_transaksi_pinjam }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->kd_anggota }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->anggota->nm_anggota }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->tg_pinjam }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->tgl_bts_kembali }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->kd_koleksi }}</td>
                                        <td class="border border-green-600 dark:border-green-400 px-4 py-2">{{ $d->koleksi->judul }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
