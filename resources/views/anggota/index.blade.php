<x-app-layout>
                    ...
                    <table id="myDataTable"
                        ...
                        <thead style="background-color: #5A827E;" class="text-white">
                            <tr>
                                ...
                                <th class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-left">Jenis
                                    Kelamin</th>
                                ...
                                <th class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-center">Jumlah
                                    Pinjam</th>
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $d)
                                <tr class="hover:bg-[#B9D4AA] transition duration-150">
                                    ...
                                    <td class="px-4 py-2 border border-gray-300 dark:border-gray-600">{{ $d->kd_anggota }}
                                    </td>
                                    <td class="px-4 py-2 border border-gray-300 dark:border-gray-600">{{ $d->nm_anggota }}
                                    </td>
                                    <td class="px-4 py-2 border border-gray-300 dark:border-gray-600">
                                        {{ $d->jk === 'L' ? 'Laki-laki' : 'Perempuan' }}
                                    </td>
                                    <td class="px-4 py-2 border border-gray-300 dark:border-gray-600 max-w-xs truncate">
                                        {{ $d->alamat }}</td>
                                    ...
                                    <td class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-center space-x-2">
                                        <button
                                            onclick="return updateData('{{ $d->id }}','{{ $d->nm_anggota }}','{{ $d->jk }}','{{ $d->alamat }}','{{ $d->status }}','{{ $d->role_id }}','{{ $d->role->role_name }}','{{ route('anggota.update', $d->id) }}')"
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg p-6 w-full max-w-md shadow-xl">
            ...
            <form id="deleteForm" method="post">
                ...
                <div class="flex justify-end gap-2">
                    <button type="submit"
                        class="bg-[#5A827E] hover:bg-[#4b6f6b] text-white px-4 py-2 rounded">Hapus</button>
    ...
    <script>
        ...
        const roles = @json($roles); // Pass roles data to JavaScript

        function addData() {
            const modalContent = document.getElementById("modal-content");
            modalContent.innerHTML = `
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Kode Anggota</label>
                <input type="text" name="kd_anggota" class="input w-full mb-2 border border-gray-300 dark:bg-gray-700 dark:text-white rounded-md p-2" value="${kodeAnggotaBaru}" readonly />
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Nama Anggota</label>
                <input type="text" name="nm_anggota" class="input w-full mb-2 border border-gray-300 dark:bg-gray-700 dark:text-white rounded-md p-2" required />
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Jenis Kelamin</label>
                ...
                <textarea name="alamat" class="input w-full mb-2 border border-gray-300 dark:bg-gray-700 dark:text-white rounded-md p-2"></textarea>
        ...
        function updateData(id, nama, jk, alamat, status, roleId, roleName, routeUrl) {
            const modalContentUpdate = document.getElementById("modal-content-update");
            modalContentUpdate.innerHTML = `
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Nama Anggota</label>
                <input type="text" name="nm_anggota" class="input w-full mb-2 border border-gray-300 dark:bg-gray-700 dark:text-white rounded-md p-2" value="${nama}" required />
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Jenis Kelamin</label>
                <div x-data="{ open: false, selected: '${jk === 'L' ? 'Laki-laki' : 'Perempuan'}', value: '${jk}' }" class="relative w-full mb-2">
                    <input type="hidden" name="jk" x-model="value" required>
                    <button type="button" @click="open = !open"
                        class="w-full px-4 py-2 rounded-md border border-[#5A827E] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#5A827E] hover:border-[#40615e] hover:bg-[#e6f0ee] dark:hover:bg-[#3b5956] transition text-left">
                        <span x-text="selected || 'Pilih...'"></span>
                ...
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Alamat</label>
                <textarea name="alamat" class="input w-full mb-2 border border-gray-300 dark:bg-gray-700 dark:text-white rounded-md p-2">${alamat}</textarea>
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Status</label>
                <div x-data="{ open: false, selected: '${status}', value: '${status}' }" class="relative w-full mb-2">
                    <input type="hidden" name="status" x-model="value" required>
                    <button type="button" @click="open = !open"
                        class="w-full px-4 py-2 rounded-md border border-[#5A827E] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#5A827E] hover:border-[#40615e] hover:bg-[#e6f0ee] dark:hover:bg-[#3b5956] transition text-left">
                        <span x-text="selected || 'Pilih...'"></span>
                ...
                <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Role</label>
                <div x-data="{ open: false, selected: '${roleName}', value: '${roleId}' }" class="relative w-full mb-2">
                    <input type="hidden" name="role_id" x-model="value" required>
                    <button type="button" @click="open = !open"
                        class="w-full px-4 py-2 rounded-md border border-[#5A827E] bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#5A827E] hover:border-[#40615e] hover:bg-[#e6f0ee] dark:hover:bg-[#3b5956] transition text-left">
                        <span x-text="selected || 'Pilih Role...'"></span>
        ...
        function deleteData(id, nama, routeUrl) {
            ...
            deleteMessage.textContent = `Apakah Anda yakin ingin menghapus data anggota "${nama}"? Data yang dihapus tidak dapat dikembalikan.`; // Added more definitive message
