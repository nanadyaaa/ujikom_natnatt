                        ...
                        <tbody class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">
                            ...
                            @forelse ($data as $d)
                                <tr class="hover:bg-[#cfe2df] dark:hover:bg-[#3b5956] transition">
                                    ...
                                    {{-- Menggunakan null coalescing operator untuk mencegah error jika relasi tidak
                                    ditemukan --}}
                                    <td class="border border-[#cfe2df] px-3 py-2">{{ $d->anggota->nm_anggota ?? 'N/A' }}
                                    </td>
                                    ...
                                    {{-- Data Status dan Aksi harus selalu ada di tampilan Admin --}}
                                    <td class="border border-[#cfe2df] px-3 py-2">{{ $d->status }}</td>
                                    <td class="border border-[#cfe2df] px-3 py-2 text-center space-x-2">
                                        <button
                                            onclick="updateData('{{ $d->id }}','{{ $d->kd_anggota }}','{{ $d->tg_pinjam }}','{{ $d->tgl_bts_kembali }}','{{ $d->kd_koleksi }}','{{ $d->status }}','{{ route('trsPinjam.update', $d->id) }}')"
                            ...
                            @empty
                                <tr>
                                    ...
                                    <td colspan="10" class="text-center py-4 text-gray-600 dark:text-gray-400">Data Not
                                        Found</td>
                                </tr>
                            @endforelse
                        </tbody>
...
**Setelah Anda melakukan langkah `dd($data->toArray());` dan melaporkan hasilnya, kita bisa melanjutkan untuk
